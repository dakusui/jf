...
the industry has been trying to address the same problem via similar yet different approaches.
...
These notations sometimes have features to make data texts non-redundant as much as possible by allowing to define nodes based on others.
...
Without these, users will need to define the same values and structures multiple times, which is error-prone and causes poor user experiences.
...
However, in this situation, where each notation and application has its own syntax sugars, user experiences across applications become inconsistent.

In this section, we will discuss a novel processing pipeline, where human-readability and machine-readability are considered separate concerns and both achieved.
To implement this approach, we have created a tool called **jq-front**<<jq-front>> and a pipeline that employs it.

This enables a single uniformed yet sufficiently powerful set of syntax sugars and various human-readable notations at the same time in a consistent manner.
...
They will just choose one of the preferred formats from **YAML**, **TOML**, etc. and define built-in files to ease user's effort if the need be.
...
In the implementation of **jq-front**,
