...
**Evaluation and Results:** We will then show case-study results, where the method is applied to a few real-world examples, such as **TOML** used in `pyproject.toml`, tests written in **Gherkin** and **YAML**.
Lastly, it will be discussed what benefits will be delivered to users of an application that takes the proposed approach and the developers of the application.
