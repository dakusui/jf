...
This is especially frequent in configurations of distributed systems and testings.
...
In this article, we strictly identify a notation's human-readability and capability to make documents essential and concise are separate concerns.
Then, we introduce a notation which focuses on the latter, called **JSON{plus}{plus}**.
Based on it, we demonstrate a pipeline-based approach is possible, where data defined in human-readability focused notations such as **YAML** and **HCL** are transformed into **JSON{plus}{plus}**, then they are passed to applications.
